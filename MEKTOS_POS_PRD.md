...
**Cambios v1.4:** Se documenta la marca del software (NAP POS), el paquete raíz real (`com.nap.pos`) y todos los módulos implementados desde v1.3: Setup Wizard inicial, recuperación de contraseña por email, ConfiguracionTienda, Categorias/Subcategorias, jerarquía de categorías en Producto, IVA y régimen tributario colombiano, numeración de comprobantes, inventario anual programable, stock mínimo global, sistema de notificaciones, Dashboard, sidebar colapsable y Design System documentado.

---

## 1. Resumen Ejecutivo

MEKTOS POS es un sistema de punto de venta de escritorio diseñado para la tienda física MEKTOS, un negocio que comercializa papelería, productos de aseo personal y alimentos. El software se comercializa bajo la marca **NAP POS**. El sistema opera 100% en modo offline, está instalado localmente en Windows y está construido con una arquitectura limpia orientada al crecimiento futuro (multi-caja, nube).
...
- Guiar al administrador en la configuración inicial del negocio mediante un asistente de setup.
- Permitir la recuperación segura de contraseñas mediante código de verificación por correo electrónico.
...
- Asistente de configuración inicial guiado (Setup Wizard de 4 pasos) al primer arranque.
- Autenticación de usuarios con control de roles.
- Recuperación de contraseña mediante código de 6 dígitos enviado al correo del negocio.
- Gestión de productos (CRUD): código de barras, nombre, precio de venta (calculado automáticamente), precio de compra, proveedor principal, ajuste de porcentaje, subcategoría, stock.
...
- Apertura y cierre de caja con cálculo de totales; la caja registra el usuario que la abrió.
- Generación de ticket de venta (impresora térmica) con numeración configurable (prefijo + número).
...
- Configuración tributaria del negocio (régimen ordinario, RST o no responsable de IVA) con manejo de IVA en ventas.
- Dashboard con métricas clave: productos activos, sin stock, clientes, estado de caja y alertas.
- Sistema de notificaciones internas: alertas de stock bajo, etc.
- Inventario anual programable (mes y día configurables).
- Importación masiva de productos desde archivo externo.
...
- El mensaje de error en login es deliberadamente genérico ("Usuario o contraseña incorrectos") para no revelar si el usuario existe.

### 6.2 Recuperación de Contraseña

- El usuario puede solicitar recuperar su contraseña desde la pantalla de login.
- El sistema genera un código de 6 dígitos de un solo uso y lo envía al correo electrónico configurado en la tienda.
- El código expira a los 15 minutos; se invalida automáticamente tras ser usado o al expirar.
- Los códigos se almacenan en memoria (no en base de datos) — un reinicio de la app los limpia.
- Solo el rol ADMIN puede iniciar la recuperación de contraseña (el cajero debe solicitarla al administrador).

### 6.3 Setup Wizard (Configuración Inicial)

- Al primer arranque, el sistema detecta que no hay configuración y lanza el Setup Wizard.
- El wizard tiene 4 pasos guiados:
  1. **Tipo de persona:** Natural o Jurídica (determina los campos requeridos).
  2. **Datos del negocio:** nombre de la tienda, datos del propietario (nombre/apellido/cédula si es natural; razón social/NIT/representante legal si es jurídica), teléfono (obligatorio) y correo (opcional).
  3. **Configuración operativa:** logo, dirección, stock mínimo global, régimen tributario e IVA, precio con IVA incluido, porcentaje de ganancia global, prefijo y número inicial de comprobantes, fecha de inventario anual (mes y día).
  4. **Credenciales del administrador:** username (generado automáticamente como inicial+apellido, editable), contraseña con confirmación y toggle de visibilidad.
- Al finalizar, el sistema crea el usuario administrador y guarda la configuración. Luego redirige al login.
- Solo una instancia de configuración existe (id = 1).

### 6.4 Configuración Tributaria

- El ADMIN configura el régimen tributario del negocio durante el Setup Wizard.
- Regímenes disponibles: **Régimen Ordinario**, **Régimen Simple de Tributación (RST)** y **No Responsable de IVA**.
- Las personas jurídicas no pueden ser "No responsable de IVA".
- Si el negocio es responsable de IVA (Ordinario o RST responsable), se configura el porcentaje de IVA (0%, 5% o 19%).
- La opción `precioConIvaIncluido` indica si el `precioVenta` almacenado ya incluye el IVA o si se suma al total.
- Si el negocio es "No responsable de IVA", el IVA pagado en compras se trata como costo del producto.

### 6.5 Gestión de Categorías y Subcategorías

- Crear, editar y desactivar categorías.
- Cada categoría puede tener múltiples subcategorías.
- Crear, editar y desactivar subcategorías dentro de una categoría.
- Los productos se asocian a una subcategoría (y por ende a una categoría).
- La asociación a subcategoría es opcional — un producto puede existir sin subcategoría.

### 6.6 Gestión de Productos

- Crear, editar, desactivar y consultar productos.
- Cada producto tiene: código de barras, nombre, precio de venta, precio de compra, proveedor principal, ajuste de porcentaje propio, subcategoría, stock y estado activo.
...
- Si un producto no tiene proveedor asignado, el sistema usa el `porcentajeGananciaGlobal` de la configuración de la tienda.

### 6.7 Gestión de Clientes
...
- Al abrir caja: registrar fecha, hora, monto inicial y usuario que la abre.
...
### 6.10 Registro de Compras de Inventario
...
### 6.11 Impresión de Tickets

- Al finalizar cada venta se debe imprimir automáticamente un ticket en impresora térmica.
- El ticket debe incluir: nombre del negocio, fecha y hora, lista de productos, cantidades, precios unitarios, total, método de pago y número de comprobante (prefijo + número secuencial).

### 6.12 Dashboard y Notificaciones

- La pantalla principal muestra un dashboard con métricas en tiempo real:
  - Total de productos activos en inventario.
  - Productos sin stock (count y estado semántico).
  - Total de clientes registrados.
  - Estado actual de la caja (abierta/cerrada).
- Un sistema de notificaciones internas genera alertas automáticas, incluyendo:
  - Productos con stock igual o inferior al `stockMinimoGlobal` configurado.
- El ADMIN visualiza las notificaciones desde la barra superior de la ventana principal.

### 6.13 Importación Masiva de Productos

- El sistema permite importar productos en lote desde un archivo externo (CSV/Excel).
- Esta funcionalidad es exclusiva del rol ADMIN.
...
| **Usabilidad** | Todas las ventanas son redimensionables (mínimos definidos); ninguna usa `setResizable(false)` |
...
**ConfiguracionTienda** *(singleton — id siempre 1)*
- `id`, `tipoPersona` *(NATURAL / JURIDICA)*, `nombreTienda`
- Si `NATURAL`: `nombre`, `apellido`, `cedula`
- Si `JURIDICA`: `razonSocial`, `nit`, `representanteLegalNombre`, `representanteLegalApellido`
- `telefono`, `correo` *(opcional)*, `direccion`, `rutaLogo` *(opcional)*
- `stockMinimoGlobal` *(umbral de alerta, por defecto 5)*
- `regimenTributario` *(REGIMEN_ORDINARIO | REGIMEN_SIMPLE | NO_RESPONSABLE_IVA)*
- `ivaPorDefecto` *(0, 5 o 19; 0 si no es responsable de IVA)*
- `precioConIvaIncluido` *(bool: si el precioVenta ya incluye IVA)*
- `porcentajeGananciaGlobal` *(% de ganancia por defecto para productos sin proveedor, por defecto 30)*
- `prefijoComprobante` *(ej: "FAC-")*, `numeroInicialComprobante` *(ej: 1)*
- `mesInventarioAnual` *(1-12, null si no configurado)*, `diaInventarioAnual` *(1-31, null si no configurado)*
- `propietarioId` *(ID del usuario ADMIN que completó el setup)*
...
- `id`, `codigoBarras`, `nombre`, `precioVenta`, `precioCompra`, `proveedorPrincipal`, `ajusteProducto` *(decimal, por defecto 0)*, `subcategoria` *(opcional)*, `stock`, `activo`
...
- `id`, `fechaApertura`, `fechaCierre`, `montoInicial`, `montoFinal`, `estado (ABIERTA / CERRADA)`, `usuario` *(quién la abrió)*
- Regla: solo una caja puede estar abierta en cualquier momento.

**Usuario**

- `id`, `username`, `passwordHash`, `rol (ADMIN / CAJERO)`, `activo`, `nombre` *(opcional)*, `apellido` *(opcional)*, `creadoPorId` *(null para el admin creado en el wizard)*
- El nombre para mostrar es `"Nombre Apellido"` si están definidos; `username` como fallback.
...
**Notificacion** *(record de dominio — no se persiste)*

- `titulo`, `mensaje`, `tipo (TipoNotificacion)`, `severidad`
...
### Nomenclatura y Marca

| Concepto | Valor |
| -------- | ----- |
| **Nombre de marca (UI)** | NAP POS |
| **Cliente / negocio** | MEKTOS |
| **Paquete raíz Java** | `com.nap.pos` |
...
| Iconografía | Ikonli FontAwesome 5 (`ikonli-javafx:12.3.1`) |
| Build tool | Gradle |
...
El sistema visual está documentado en `.interface-design/system.md` y define:

- **Paleta:** Superficies oscuras de carbón (`#1E2A3A`) para sidebar/headers y papel cálido (`#F5F1EB`) para contenido — metáfora "mostrador/recibo".
- **Tipografía:** `"Segoe UI", "Arial", sans-serif`; base 13px.
- **Espaciado:** Unidad base 4px; sidebar 220px (colapsado 60px).
- **Radios:** Controles 8px, cards 12-14px, badges 16-20px.
- **Iconografía:** Ikonli FontAwesome 5 (`FontIcon`).
- **Stylesheets:** `styles.css` (global), `setup_wizard.css` (wizard + login), `importacion.css` (importación).
- **Ventanas:** Todas redimensionables. Mínimos: wizard 700×560, login 520×480, ventana principal 1100×700.
...
| ---- | ----------- | ------ |
| **Fase 1** | Configuración base del proyecto | ✅ Completado |
| **Fase 2** | Creación de entidades y repositorios | ✅ Completado |
| **Fase 3** | Implementación de servicios | ✅ Completado |
| **Fase 4** | Desarrollo de UI — Setup Wizard, Login, Main Window (Dashboard) | ✅ En progreso — wizard, login y dashboard implementados |
| **Fase 4b** | Desarrollo de UI — Módulos internos: Ventas, Inventario, Compras, Clientes, Configuración | En curso |
| **Fase 5** | Integración impresora térmica | Pendiente |
| **Fase 6** | Validación y pruebas de seguridad | Pendiente |
| **Fase 7** | Empaquetado con jpackage | Pendiente |
...
- [ ] Al primer arranque sin configuración, el sistema lanza el Setup Wizard y no permite usar la app hasta completarlo.
- [ ] El Setup Wizard crea el usuario administrador con contraseña hasheada en BCrypt.
- [ ] El código de recuperación de contraseña expira en 15 minutos.
- [ ] El código de recuperación se invalida tras ser usado exitosamente.
- [ ] Ninguna ventana usa `setResizable(false)`.
- [ ] La caja registra el usuario que la abrió.
- [ ] El dashboard muestra productos activos, sin stock, clientes y estado de caja en tiempo real.
- [ ] El sistema genera alertas cuando el stock de un producto cae al `stockMinimoGlobal` o por debajo.
- [ ] El ticket de venta incluye el número de comprobante formateado como `prefijo + número secuencial`.
- [ ] Las personas jurídicas no pueden seleccionar el régimen "No responsable de IVA".
...
| Código de recuperación de contraseña interceptado | Baja | Alto | Código de 6 dígitos con expiración de 15 min; solo se envía al correo del negocio configurado |
...
| Término | Definición |
|---------|------------|
| **NAP POS** | Nombre de marca del software de punto de venta |
...
| **Setup Wizard** | Asistente de configuración inicial de 4 pasos que se ejecuta al primer arranque |
| **ConfiguracionTienda** | Entidad singleton que almacena toda la configuración operativa del negocio |
| **RegimenTributario** | Clasificación tributaria del negocio: Régimen Ordinario, RST o No Responsable de IVA |
| **StockMinimoGlobal** | Umbral configurado en la tienda por debajo del cual se generan alertas de stock |
| **PorcentajeGananciaGlobal** | Margen de ganancia por defecto que aplica a productos creados sin proveedor asignado |
| **Notificacion** | Alerta interna del sistema (stock bajo, etc.) mostrada en el dashboard |
| **Categoria / Subcategoria** | Jerarquía de clasificación de productos (Categoria → Subcategoria → Producto) |
| **Design System** | Conjunto de tokens de color, tipografía, espaciado y componentes que rigen la UI de NAP POS |

---

*Documento preparado como referencia base para el desarrollo de MEKTOS POS v1.0 (NAP POS).*
